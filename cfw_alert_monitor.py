...
def normalize_direct_result(value):
    ...
    if value in ("复核", "疑似攻击", "封禁候选", "优先排查"):
        return "需人工复核"
    return "未见成功证据" if value in ("观察",) else "需人工复核"
...
def call_codex_batch(config, model, batch):
    ...
    if direct_alert:
        task_text = (
            ...
            "模型研判取值:确认成功/确认未成功/未见成功证据/扫描探测/业务误报/需人工复核。"
            "业务误报=内网业务/正常调用被规则误命中(请求是业务数据非攻击载荷);"
            "未见成功证据=有真实攻击载荷但未见得手。两者要分清。"
...
def codex_direct_prompt(batch, id_map):
    ...
    if direct_alert:
        task = (
            ...
            "确认成功,确认未成功,未见成功证据,扫描探测,业务误报,需人工复核。"
            "业务误报=请求是正常业务数据(业务URL/参数)被规则误命中;"
            "未见成功证据=请求含真实攻击载荷但未见得手。看 req 是业务数据还是攻击payload 来区分。"
