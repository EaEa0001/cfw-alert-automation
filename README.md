...
- **企微**:继续发小时/日报汇总(`wecom` 配置)。
- **微信(weclaw / WeChat iLink bot)**:专推"需人工复核"和"确认成功"的告警卡片到个人微信,误报不打扰。复用本机已登录的 weclaw bot,通过 `weclaw send` CLI 发送。

```powershell
# 首次或登录态过期时,扫码登录 bot:
C:/Users/<you>/go/bin/weclaw.exe login
C:/Users/<you>/go/bin/weclaw.exe start
# 测试:
python .\weclaw_notify.py --text "通道测试"
```

`config.json` 的 `weclaw`:`enabled`(开关)、`max_items`(单条消息最多列几条)、`to_user_id`(留空发给 bot 绑定的本人)。登录态过期会返回 `session timeout` 提示,重新 `weclaw login` 即可。
